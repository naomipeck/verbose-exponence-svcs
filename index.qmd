...
::: {.callout-caution}
This site represents a work in progress! Please do not directly cite any text without asking me first, although reference to the ideas here is more than fine.
:::
...
While aspect can be marked via multiple exponence, tense is only marked once per construction.

(@saramaccan4) @byrne1990, 153, example 10
```{r}
#| label: saramaccan-tense2
gloss_example(transliteration = "Kófi bì bái dí búku dá dí muyée",
             glosses = "Kofi PST buy the book give the woman",
             free_translation = "Kofi bought the book (and then) gave (it) to the woman.")
```
...
Taba is an example of a language for which Aikhenvald identifies "optional" concordant marking for subject agreement [@aikhenvald2018, 101].

(@taba1) @bowden2001, 295, example 1
```{r}
#| label: taba5
gloss_example(transliteration = "n=han ait te-su",
             glosses = "3SG=go ascend NEG-POT",
             free_translation = "(S)he hasn't gone up yet.",
             annotation = "Nhan ait tesu.")
```

(@taba2) @bowden2001, 300, example 19
```{r}
#| label: taba2
gloss_example(transliteration = "n=han n=ait te-su",
             glosses = "3SG=go 3SG=ascend NEG-POT",
             free_translation = "(S)he hasn't gone up yet.",
             annotation = "Nhan nait tesu.")
```

> I have not been able to find any difference in meaning between the two types of structure. Neither are there any intonational differences. The construction with cross-referencing of both verbs in the sequence appears to be simply correlated with slower, more careful speech registers while that with cross-referencingo f only the first verb occurs in faster, more careful speech. A detailed study of the discourse contexts in which one or the other pattern of cross-referencing is used might throw further light on the matter... [@bowden2001, 300]
...
(@morisyen1) @syea2013, 18, example 2e 
```{r}
#| label: morisyen
...
(@goemai3) @hellwig2006, 91, example 2e
```{r}
#| label: goemai-3
gloss_example(transliteration = "mûep swo / mûep rwo n-s'et",
             glosses = "3PL run.PL {} 3PL enter.PL LOC-bush",
...
(@goemai4) @hellwig2006, 95, example 7a
```{r}
#| label: goemai-4
gloss_example(transliteration = "mutane d'e t'ong s'arap yi t'ong s'oe",
             glosses = "people.PL exist.PROG PROG trade.PL PROG PROG eat",
             free_translation = "People are trading (them and) eating (them).")
```
...
(@ftotonac2) @mcfarland2009, 201, example 249
```{r}
#| label: ftotonac-2
gloss_example(transliteration = "ka-tan-ti̥ tantlii-aa-[cg]",
...
(@numbami) @bradshaw1993, 152, example 56b
```{r}
#| label: numbami
...
-   subset exponence 
    ...
    → less specific (i.e. less informative) markers more likely to be lost? [cf. @grofulovicmuller2023]

# Conclusions

-   moving from a production-based to a processing-based typology helps us generate hypotheses of how different marking patterns in SVCs may actually be used
